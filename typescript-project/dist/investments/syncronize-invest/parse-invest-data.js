...
import { normalizeDate, normalizeCurrency } from "../common/common-helpers.js";
export const parseBankList = () => {
    const inputDataFile = process.argv[2];
    if (!inputDataFile) {
        throw new Error("Input data file path is required");
    }
    const investFlatList = [];
    const investList = JSON.parse(readFileSync(inputDataFile, 'utf8'));
...
export async function parseBaseData(doc) {
    await doc.loadInfo();
...
//# sourceMappingURL=parse-invest-data.js.map