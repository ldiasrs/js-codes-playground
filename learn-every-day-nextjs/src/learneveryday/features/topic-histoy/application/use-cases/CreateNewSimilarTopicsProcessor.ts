...
export class CreateNewSimilarTopicsProcessor {
  ...
  private async getExistingSubjects(customerId: string): Promise<Set<string>> {
    ...
    return new Set<string>(existingTopics.map((t: Topic) => t.subject.trim().toLowerCase()));
  }
  ...
  private async tryCreateTopic(customerId: string, subject: string): Promise<Topic | undefined> {
    try {
      const createdDTO = await this.addTopicFeature.execute({ customerId, subject });
      const created = await this.topicRepository.findById(createdDTO.id);
      if (!created) {
        throw new Error(`Failed to retrieve created topic ${createdDTO.id}`);
      }
  ...
  private async handleTierLimitAndRetry(customerId: string, subject: string): Promise<Topic | undefined> {
    try {
      ...
      const createdAfterDeleteDTO = await this.addTopicFeature.execute({ customerId, subject });
      const createdAfterDelete = await this.topicRepository.findById(createdAfterDeleteDTO.id);
      if (!createdAfterDelete) {
        throw new Error(`Failed to retrieve created topic ${createdAfterDeleteDTO.id}`);
      }
  ...
  private async findOldestTopic(customerId: string): Promise<Topic | undefined> {
    ...
    return topics.reduce((oldest: Topic, t: Topic) => (t.dateCreated < oldest.dateCreated ? t : oldest), topics[0]);
  }
